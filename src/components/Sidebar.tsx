...
export function Sidebar({ collapsed, mobileOpen, onCloseMobile }: Props) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex h-screen w-52.5 shrink-0 flex-col overflow-hidden border-r border-slate-200 bg-white transition-transform duration-200 md:visible md:sticky md:top-0 md:z-20 md:translate-x-0 md:transition-[width] ${mobileOpen ? "visible translate-x-0" : "invisible -translate-x-full"} ${collapsed ? "md:w-16" : "md:w-52.5"}`}
      >
      <div className="flex h-15.5 shrink-0 items-center justify-center px-3">
        <Logo variant="wordmark" className="h-13 w-full md:hidden" />
        <Logo variant={collapsed ? "mark" : "wordmark"} className={`hidden md:block ${collapsed ? "h-8 w-8" : "h-13 w-full"}`} />
        ...
        {NAV_GROUPS.map((group) => (
          <div key={group.label} className="mb-3">
              <button
                ...
                className={`flex w-full items-center justify-between rounded-md px-1.5 pb-2 pt-1 text-left text-xs font-medium tracking-[0.04em] text-slate-500 transition-colors hover:bg-slate-50 hover:text-slate-700 ${collapsed ? "md:hidden" : ""}`}
              >
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `mb-0.5 flex h-6.75 items-center gap-2.5 rounded-md text-2xs font-medium no-underline transition-colors ${
                    collapsed ? "ml-2 px-1.5 md:ml-0 md:justify-center md:px-2" : "ml-2 px-1.5"
