  ...
  return <div className={containerClassName}>
    <div className="overflow-x-auto">
      <table className={ui.cx(ui.table, tableClassName)}>
        ...
        <tbody className={isLoading && rows.length > 0 ? "opacity-60 transition-opacity" : "transition-opacity"}>
          ...
          {showLoadingRow && <tr><td colSpan={columns.length} className={ui.cx(ui.td, "py-8 !text-center text-slate-500")}><span className="inline-flex items-center gap-2"><LoaderCircle size={14} className="animate-spin text-blue-600" />Loading data...</span></td></tr>}
    ...
    {pagination && <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 px-4 py-3">
      <div className="flex flex-wrap items-center gap-3">
        ...
        {isLoading && rows.length > 0 && <span role="status" className="inline-flex items-center gap-2 text-xs text-slate-500"><LoaderCircle size={14} className="animate-spin text-blue-600" />Loading data...</span>}
      </div>
